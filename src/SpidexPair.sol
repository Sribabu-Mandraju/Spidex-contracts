...
contract SpidexPair is SpidexERC20 {
    ...
    //               ERRORS
    /////////////////////////////////////////////////
    error SpidexPair__Unauthourized(address caller);
    error SpidexPair__TransferFailed(address token, address to);
    error SpidexPair__InsufficientLiquidity();
    error SpidexPair__Overflow();
    error SpidexPair__InsufficientAmount();
    error SpidexPair__InsufficientBalance();
    error SpidexPair__InvalidAddress();
    error SpidexPair__InvalidUniswap_K();
    ...
    function initialize(address tokenA, address tokenB) external {
        require(msg.sender == factory, SpidexPair__Unauthourized(msg.sender));
        token0 = tokenA;
        token1 = tokenB;
    }
    ...
    function _safeTransfer(address token, address to, uint256 amount) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), SpidexPair__TransferFailed(token, to));
    }
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        require(liquidity > 0, SpidexPair__InsufficientLiquidity());
        _mint(to, liquidity);
        _update(balance0, balance1, reserveA, reserveB);
        emit SpidexPair__Mint(to, liquidity);
    }
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, SpidexPair__InsufficientLiquidity());
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes memory data) external lock {
        require(amount0Out > 0 || amount1Out > 0, SpidexPair__InsufficientAmount());
        (uint112 _reserve0, uint112 _reserve1,) = getReserves();
        require(amount0Out <= _reserve0 && amount1Out <= _reserve1, SpidexPair__InsufficientBalance());
        ...
        uint256 balance1;
        {
            address _token0 = token0;
            address _token1 = token1;
            require(to != _token0 && to != _token1, SpidexPair__InvalidAddress());
        ...
        require(amount0In > 0 || amount1In > 0, SpidexPair__InsufficientAmount());
        {
            uint256 balance0Adjusted = (balance0 * 1000) - (amount0In * 3);
            uint256 balance1Adjusted = (balance1 * 1000) - (amount1In * 3);
            require(
                balance0Adjusted * balance1Adjusted >= uint256(_reserve0) * uint256(_reserve1) * (1000 ** 2),
                SpidexPair__InvalidUniswap_K()
            );
        }
        _update(balance0, balance1, _reserve0, _reserve1);
        emit SpidexPair__Swap(to, amount0In, amount1In, amount0Out, amount1Out);
    }
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) internal {
        require(balance0 <= type(uint112).max && balance1 <= type(uint112).max, SpidexPair__Overflow());
        ...
        emit SpidexPair__ReserveUpdated(reserve0, reserve1, blockTimeStampLast);
    }
